...
import { useCanPublish } from '../../auth/SupabaseAuthContext';
...
export function AdminPublicationsPage() {
  ...
  return (
    <section>
      ...
      {!canPublish ? <p className="admin-inline-notice" role="note">Mode analyste : consultation autorisée, publication, retrait et restauration réservés au rôle éditeur.</p> : null}
      {state.data.length ? (
          ...
          {state.data.map((item) => {
            ...
            return (
              <article className="admin-publication-card" key={item.publication_id}>
                ...
                {canPublish && (item.state === 'PUBLISHED' || item.state === 'WITHDRAWN') ? (
