...
export function AdminIncidentSpatialReviewPage({ fireId }: { readonly fireId: string }) {
  ...
  const resumeRevision = (revision: AdminActiveFireZoneRevision) => {
    ...
    setDraft(points); setMovingDraftIndex(null); setSupportingMarkers(revision.supporting_marker_ids); if (revision.analysis_id) setSelectedAnalysisId(revision.analysis_id); setDrawMode(true); setMessage('Calque chargé dans l’éditeur. Modifiez ses sommets directement sur le terrain.');
  };
  ...
  const saveDraft = () => {
    ...
    void mutate(() => api.createActiveFireZoneRevision(fireId, { expected_latest_revision: latestRevision, valid_at: validAt, analysis_id: selectedAnalysisDay?.analysis_id, geometry_geojson: { type: 'Polygon', coordinates: [ring] }, supporting_marker_ids: supportingMarkers, reason }, { idempotencyKey: key('active-zone') }), selectedAnalysisDay ? `Calque du ${selectedAnalysisDay.local_date} enregistré. Il reste privé jusqu’à sa validation.` : 'Calque enregistré. Il reste privé jusqu’à sa validation.').then((saved) => { if (saved) { setDraft([]); setMovingDraftIndex(null); setDrawMode(false); } });
  };
